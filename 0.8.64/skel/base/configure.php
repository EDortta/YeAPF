...
if (!$isCLI) {
	echo "<!doctype html>\n\n<html><head><meta name='viewport' content='width=device-width, initial-scale=1' /><link rel='stylesheet' href='https://cdnjs.cloudflare.com/ajax/libs/mini.css/3.0.1/mini-default.min.css'>
...
echo sayStep("<div style='border-left: solid 4px black; padding: 12px; background-color: #fff'>
    ...
    Last modification: 2020-02-26 11:39:26 (-3 DST)</h3></div>");
...
$ySearchPath = "includes;YeAPF/includes;.distribution/0.8.64/includes;YeAPF/.distribution/0.8.64/includes;lib;lib/YeAPF;lib/YeAPF/includes";
...
if ($canConfig) {
	...
	if (function_exists("yeapfVersion")) {
		...
		if (("0.8.64") != yeapfVersion()) {
			dieConfig(sayStep("<span class=redDot></span><ul class=err>Your configure version is '0.8.64' while your installed version is '" . yeapfVersion() . " at " . yeapfBaseDir() . "'<br><small>You can use 'yeapf.path' file to indicate where is your YeAPF distribution</small></ul>"));
		}
	...
	if ((is_writable('./')) && (touch('flag.test'))) {
		...
		if (lock('configure')) {
			...
			*/
			...
			$yeapfDBPath .= ',' . getSgugPath(dirname(getenv('SCRIPT_FILENAME')));
			...
			*/
			...
			if ($canConfig) {
				...
				if (($DB_CSV_PATH == '') || ($recreateCSVFile)) {
					if ($DB_CSV_PATH == '') {
						if ($YEAPF_INI_PATH == '') {
							$dbCSVFilename = dirname(getenv('SCRIPT_FILENAME')) . "/db.csv";
						} else {
				...
				*/
				...
				if ($auxDefaultTimeZone != 'UTC') {
					if (date_default_timezone_set($auxDefaultTimeZone)) {
						...
						/* flags/flag.nodb */
						if (($YEAPF_INI_PATH > '') || ($recreateCSVFile) || (filesize($dbCSVFilename) == 0)) {
                    ...
                    */
                    ...
                    if (function_exists("yeapfVersion"))
                      if (("%"."YEAPF_VERSION%")==yeapfVersion()) {
                        ...
                        if (("0.8.64") != yeapfVersion())
                          _yLoaderDie(true, "Your configure version is \'0.8.64\' while your installed version is \'".yeapfVersion()."\'");
                      }
                    if (!isset($appName))
                      $appName = "dummy";
