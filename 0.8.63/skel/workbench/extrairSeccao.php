...
if ($argc > 1) {
	...
	if (file_exists($filename)) {
		...
		if ($argc > 1) {
			...
			$newname = str_replace(".$ext", ".min.html", $filename);
		}

		echo "Gerando $newname\n";
		if (!file_exists($newname)) {
			file_put_contents($newname, $html_out);
		} else {
			die("ERRO: O arquivo '$newname' já existe\n");
		}

	}
} else {
	die("Indique o arquivo de entrada e o arquivo de saída.\n");
}
?>
