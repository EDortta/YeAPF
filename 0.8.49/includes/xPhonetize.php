...
*/
  ...
  class phonetize {
    ...
    {
      // pegamos sÛ as minusculas
      $this->aWord=strtolower($this->aWord);
      // eliminamos acentuaÁıes
      $this->eliminarAcentuadas();
      // pegamos sÛ as letras
      $this->aWord=ereg_replace("[^A-Z,^a-z]", "", $this->aWord);
      if ($this->aWord>'') {
        $this->passNDX=0;
        // erros de digitaÁ„o mais comuns
        $this->substPhonems();
        $this->eliminarLetrasDuplicadas();
        // simplificaÁ„o de erros menos comuns e sobra da simplificaÁ„o anterior
        $this->substPhonems();
        // elimino fonemas apÛs reduÁ„o
        $this->substPhonems();
      }
      return $this->aWord;
    }
